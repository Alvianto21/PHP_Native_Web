...
<section class="d-flex align-items-center justify-content-center py-4">
	<div class="form-article w-100 m-auto">
		<form action="<?php echo ABSOLUTURL; ?>admin/articleUpdate/<?php echo htmlspecialchars($article['slug']); ?>" method="post" enctype="multipart/form-data" id="update_article_admin">
			<h1 class="h3 mb-3 fw-normal">Edit article <?php echo htmlspecialchars(substr($article['title'] ?? '', 0, 30), ENT_QUOTES); ?></h1>
			<h2 class="h4 mb-3 fw-medium">Author: <?php echo htmlspecialchars($article['author'], ENT_QUOTES); ?></h2>
			...
			<div class="form-floating form-group mb-3">
				<select name="is_deleted" id="is_deleted" class="form-select <?php echo !empty($errors['is_deleted']) ? 'is-invalid' : ''; ?>" required>
					...
					<option value="0" <?php echo (($article['is_deleted'] ?? $old['is_deleted']) === 0) ? 'selected' : ''; ?>>Available</option>
					<option value="1" <?php echo (($article['is_deleted'] ?? $old['is_deleted']) === 1) ? 'selected' : ''; ?>>Deleted</option>
				</select>
				<label for="is_deleted" class="form-label">Article Status</label>
			...
			<button type="submit" class="btn btn-primary w-100 py-2" id="revise_article_admin">Submit</button>
		</form>
	</div>
</section>
...
<script>
	document.getElementById('update_article_admin').addEventListener('submit', async function (event) {
		// Stop action form
		event.preventDefault();
		const form = this;

		// Target submit button, input photo and photo_path
		const submitButton = document.getElementById('revise_article_admin');
